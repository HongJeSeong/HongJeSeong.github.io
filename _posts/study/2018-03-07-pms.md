...
---
# Ubuntu Software Visualzation Ubuntu
`2019-01-22 우분투 설치 기록`
`OS : Ubutu 18.04`
...
### PHP Install
```sh
apt-get install -y php php-mysql # install php phpmysql
```
...
### Redmine Install
```sh
apt-get install -y redmine redmine-mysql #install redmine
```
...
```sh
apt-get install -y libapache2-mod-jk tomcat8    #jk_moudls, tomcat8 install
...
```sh
vim /etc/apache2/mods-available/jf.conf
# <IfIModule jk_module> </IfModule> 안의 내용을 수정
```
기존의 `JKWokersFile /etc/libapache2-mod-jk/wokers.properties`를 #주석처리 후,
`JKWorkersFile /etc/apache2/workers.properties ` 로 변경
**변경 전**
![변경 전](../../images/posts/pms/tomcat_1.JPG)
**변경 후**

![변경 후](../../images/posts/pms/tomcat_2.JPG)  
...
`<Connerctor port="8009" protocol="AJP/1.3" redirectPort="8443"/> ` 부분이 주석처리가 되어 있다면, 해제한다. 이 부분이 없다면 추가한다.  
